...
public class PatientManagement {
    ...
    public static void registerPatient() {
        ...
        while (true) {
            ...
            if (newName) {
                ...
                System.out.println("\u001B[31m---->>! Name already Registered. Try Again.\u001B[0m");
            }
        }
        ...
        while (true) {
            ...
            try {
                ...
                if (date.isAfter(LocalDate.now())) {
                    System.out.println("\u001B[31m---->>! Date of birth cannot be in the future. Try Again.\u001B[0m");
                    continue;
                }
                ...
                if (checkInput > 1900 && checkInput <= LocalDateTime.now().getYear()) {
                    ...
                    System.out.println("\u001B[31m---->>! Invalid Year in Date of Birth. Try Again.\u001B[0m");
                }
            } catch (Exception e) {
                System.out.println("\u001B[31m---->>! Invalid date format. Please try again (YYYY-MM-DD)\u001B[0m");
            }
        ...
        while (true) {
            ...
            if (sex.equals("male")) {
                ...
                System.out.println("\u001B[31m---->>! Please choose between > Male < & > Female <.\u001B[0m");
            }
        }

        // get blood type
        while (true) {
            bloodType = ValidateInputs.checkInput("\u001B[33m>>\u001B[0m Enter Blood Type: ").toUpperCase();
            ...
            if (validBL) {
                ...
                System.out.println("\u001B[31m---->>! BloodType does not exist. Try Again.\u001B[0m");
            }
        }
        ...
        while (true) {
            ...
            if (phoneNumber.isEmpty()) {
                ...
                System.out.println(
                        "\u001B[31m---->>! Invalid phone number. Must be 11 digits (e.g., 09171234567). \u001B[0m");
            } else {
        ...
        while (getAllergies) {
            ...
            if (yesOrNo.equals("yes")) {
                System.out.println(
                        "\u001B[34m<<------------\u001B[35mPlease provide the \u001B[33mallergies\u001B[35m below\u001B[34m------------>>\u001B[0m");
                System.out.println("            Type and Enter \"Done\" when finished.");
                int pos = 1;
                while (true) {
                    String provideAllergies = ValidateInputs.checkInput("\u001B[33m" + pos++ + "\u001B[0m: ")
                            .toLowerCase();
        ...
        while (getMedications) {
            String yesOrNo = ValidateInputs
                    .checkInput("\u001B[33m>>\u001B[0m Does the patient have any current Medications? (Yes/No): ")
                    .toLowerCase();
            if (yesOrNo.equals("yes")) {
                System.out.println(
                        "\u001B[34m<<------------\u001B[35mPlease provide the \u001B[33mmedications\u001B[35m below\u001B[34m------------>>\u001B[0m");
                System.out.println("            Type and Enter \"Done\" when finished.");
                int pos = 1;
                while (true) {
                    String provideMed = ValidateInputs.checkInput("\u001B[33m" + pos++ + "\u001B[0m: ").toLowerCase();
        ...
        // get medical history
        boolean getMedicalHistory = true;
        while (getMedicalHistory) {
            String yesOrNo = ValidateInputs
                    .checkInput("\u001B[33m>>\u001B[0m Does the patient have any medical history? (Yes/No): ")
                    .toLowerCase();
            if (yesOrNo.equals("yes")) {
                while (true) {
                    String illness = ValidateInputs.checkInput("\u001B[33m>>\u001B[0m Enter Illness/Condition: ");
                    String treatment = ValidateInputs
                            .checkInput("\u001B[33m>>\u001B[0m Enter Treatment or Medication Given: ");
                    System.out.print("\u001B[33m>>\u001B[0mEnter Date of History (YYYY-MM-DD): ");
                    String date = Main.in.nextLine();

                    String record = String.format("Illness: %s | Treatment: %s | Date: %s", illness, treatment, date);
                    medicalHistory.add(record);

                    System.out.print("\u001B[33m>>\u001B[0m Add another medical history? (Yes/No): ");
                    String more = Main.in.nextLine().toLowerCase();
                    if (more.equals("no")) {
                        getMedicalHistory = false;
                        break;
                    }
                }
                break;
        ...
        System.out.println("\n \u001B[32mPatient Registered Successfully! \u001B[35m >>>\u001B[0m Assigned Patient ID: "
                + patientID);
        PatientRecord newPatient = new PatientRecord(patientID, fName, lName, dob, age, sex, bloodType, address,
                ...
                case 3:
                    while (true) {
                        ...
                        String searchPatient = ValidateInputs
                                .checkInput("\u001B[33m>>\u001B[0m Enter Patient Name or ID to Select/Search: ").trim();
                        ...
                        if (isPatientID) {
                            ...
                            if (found) {
                                System.out
                                        .println(
                                                "\u001B[33m>>\u001B[0m \u001B[35mOptions:\u001B[0m [1] Update Details, [2] View Medical History, [3] Exit.");
                                int options = ValidateInputs
                                        .checkIntInput(
                                                "\u001B[33m>>\u001B[0m What would you like to do next (choose between [1 - 3]): ");
                                switch (options) {
                                    case 1:
                                        System.out.println(
                                                "\u001B[34m---->>!\u001B[0m Choose Which Among the Following You Would Like to Change. \u001B[34m!<<----\u001B[0m");
                                        System.out.println(
                                                "\u001B[33m>>\u001B[0m \u001B[35mOptions:\u001B[0m [1] Update Contact Number, [2] Update Address, [3] Update Status.");
                                        int changeWhat = ValidateInputs
                                                .checkIntInput("\u001B[33m>>\u001B[0m Choose between [1 - 3]: ");

                                        switch (changeWhat) {
                                            case 1:
                                                updatePhoneNum(index);
                                                break;
                        ...
                        // linear search
                            ...
                            for (PatientRecord pr : patientRecords) {
                                if (pr.getFullName().equalsIgnoreCase(searchPatient)) {
                                    ...
                                    break;
                                } else if (pr.getFirstName().equalsIgnoreCase(searchPatient)
                                        || pr.getLastName().equalsIgnoreCase(searchPatient)) {
                                    pr.displayPatientDetails();
                                    found = true;
                                    index = patientRecords.indexOf(pr);
                                    break;
    ...
    public static void updatePhoneNum(int index) {
    ...
    public static void updateAddress(int index) {
        System.out.println("\u001B[34m---->>>\u001B[0m Updating Address \u001B[34m<<----\u001B[0m");
        System.out.println("\u001B[34m>> \u001B[0mCurrent Address: "
                + patientRecords.get(index).getAddress());
        String newAddress = ValidateInputs
                .checkInput("\u001B[33m>>\u001B[0m Enter new Address (Province): ");
